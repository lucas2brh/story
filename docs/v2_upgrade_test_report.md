...
## F1: Pending Unbonding Across Upgrade — PASS

**Date**: 2026-03-24
**Genesis**: `storyprotocol/story-devnet-aws` branch `qa/f1-unbonding-test` (unbonding_time=600s)
**Binary**: `story-f1-unbonding-9c3c3a6`

### Setup

| Step | Height | Action | Result |
|------|--------|--------|--------|
| 1 | 349 | `stake` 1024 IP to validator1 | tx 0xf34fc6... status 0x1 |
| 2 | 349 | `unstake` 1024 IP → creates unbonding entry | tx 0x2c1270... status 0x1 |
| 3 | 380 | Fund executor 1 ETH | tx 0x017142... status 0x1 |
| 4 | 458 | `planUpgrade("v2.0.0", 486)` via TimelockController | schedule 0x2f144a... + execute 0xce0cb3... both 0x1 |
| 5 | 486 | Chain halts, cosmovisor swaps binary | - |
| 6 | 491 | Blocks resume post-upgrade | catching_up: false |
| 7 | 511 | Smoke test (transfer) | 0x1 |
| 8 | 610 | Unbonding completed, stake+rewards returned | Balance +1024 IP |

### Verification

| Check | Value |
|-------|-------|
| Balance before unbonding completes | 99998973978646127990657681 |
| Balance after unbonding completes | 99999997978651541990657681 |
| Difference | +1,024,000,005,414,000,000,000 (~1024 IP + rewards) |
| Transfer tx post-unbonding | 0x1 |
| Contract call (minStakeAmount) | 1024000000000000000000 |
| Block production (5s) | 659 → 661 |

**Conclusion**: Unbonding delegation created at height 349, survived v2.0.0 upgrade at height 486, completed at height 610 (balance change confirmed at block boundary 609→610). Funds returned with staking rewards. Chain fully functional post-upgrade.
