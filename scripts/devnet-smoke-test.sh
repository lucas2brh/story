...
if [ "$STATUS" = "0x1" ]; then
  ...
  echo "   FAIL  tx: $HASH status: $STATUS"
  exit 1
fi

# 2. Contract read (UpgradeEntrypoint.planUpgrade is write-only, use a view call)
...
if [ $? -eq 0 ] && [ -n "$RESULT" ]; then
  ...
  echo "   FAIL  contract call failed: $RESULT"
  exit 1
fi

# 3. Block height sanity
