...
PROPOSER_CL=$(curl -s "$RPC/abci_query?path=\"/cosmos.auth.v1beta1.Query/AccountInfo\"&data=$(printf '%s' "0a$(printf '%02x' ${#PROPOSER_ADDR})$(echo -n "$PROPOSER_ADDR" | xxd -p -c 999)" | tr -d '\n')" 2>/dev/null | jq -r '.result.response.value' 2>/dev/null || echo "")
# Query unbonding via EVM — check if unstake was processed
UNSTAKE_PERIOD=$(cast call "0xCCcCcC0000000000000000000000000000000001" "minUnstakeAmount()(uint256)" --rpc-url "$RPC" 2>/dev/null | awk '{print $1}')
if [ -n "$UNSTAKE_PERIOD" ]; then
  echo "  Unbonding check: staking contract reachable post-upgrade"
  check "PASS" "Staking contract accessible post-upgrade"
else
  check "FAIL" "Staking contract not accessible"
fi
echo "  NOTE: With unbonding_time=600s, wait ~10 min then verify funds returned"
echo ""
